...
def display_non_iid_issues(lab, df, labels):
    ...
    if total_non_iid > 0:
        ...
        display_df = df.iloc[non_iid_indices].copy()
        display_df['target_value'] = labels.iloc[non_iid_indices]
        display_df['non_iid_score'] = non_iid_results['non_iid_score'].iloc[non_iid_indices]
        print(display_df)
    ...
    if save_option == 'y':
        ...
        save_results_to_json(non_iid_results, dataset_name, output_path=output_path, issue_type="non_iid_regression")

def main():
    try:
        print("\n=== Non-IID Checker (Regression) ===")
        ...
        pred_probs = get_pred_probs(X_processed, labels, task='regression')
